...
def miouandpa(config, output, target):
    ...
    if config.out_channels == 1:
        # binary segmentation: build 2-class probs for metric
        if target.dim() == 4 and target.size(1) == 1:
            target_bin = target.squeeze(1)
        else:
            target_bin = target
        target_bin = (target_bin > 0.5).long()

        prob = torch.sigmoid(output)
        output_two = torch.cat([1.0 - prob, prob], dim=1)

        metric = SegmentationMetric(2)
        metric.update(output_two, target_bin)
...
def predict_a_batch(net, out_channels, batch_data, batch_label, class_weights, do_criterion, do_metric):
    ...
    with torch.no_grad():
        ...
        if out_channels == 1:
            ...
            prediction_np = (output_sigmoid > 0.5).long().squeeze(1).cpu().numpy()
        else:
            batch_label = batch_label.squeeze(1)
            prediction_np = torch.max(output.data, 1)[1].cpu().numpy()
        ...
        if do_metric:
            if out_channels == 1:
                target_bin = (batch_label > 0.5).long()
                if target_bin.dim() == 4 and target_bin.size(1) == 1:
                    target_bin = target_bin.squeeze(1)
                output_two = torch.cat([1.0 - output_sigmoid, output_sigmoid], dim=1)
                metric = SegmentationMetric(2)
                metric.update(output_two, target_bin)
                pa, miou = metric.get()
            else:
...
def eval_dataset_full(net, out_channels, loader, class_weights=None, save_dir=None, is_val=True):
    ...
    if save_dir is None:
        ...
        if is_val:
            ...
            with open(save_dir+'/val_loss.txt', 'w') as t:
                for l in losses:
                    t.writelines(str(round(l,3)) + '\n')
            with open(save_dir+'/val_miou.txt', 'w') as t:
                for l in mious:
                    t.writelines(str(round(l,3)) + '\n')
            with open(save_dir+'/val_pa.txt', 'w') as t:
                for l in pas:
                    t.writelines(str(round(l,3)) + '\n')
        else:
...
def predict_images(net, args, dst_size=(512, 512), save_dir=None):
    ...
    if not os.path.exists(save_dir+'/vis'):
    ...
    for path in paths:
        ...
        dst_frame = frame.resize(dst_size, Image.BILINEAR)
        dst_prediction = cv2.resize(prediction_np, dst_size)
    ...
    if save_dir is not None:
        with open(os.path.join(save_dir, 'vis', 'predictions_every_class.json'), 'w') as f:
