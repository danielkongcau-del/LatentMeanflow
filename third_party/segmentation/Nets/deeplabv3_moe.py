...
class MobileNetV2(nn.Module):
    def __init__(self, downsample_factor=8, pretrained=True):
        ...
        self.features = model.features[:-1]  # MobileNetV2的最后一层（conv_1x1_bn）不要，并以字典的形式存储
    ...
    # 输出两个有效特征层
    def forward(self, x):
        low_level_features = self.features[:4](x)  # 输出前四层处理后的特征层
        x = self.features[4:](low_level_features)  # 输出全部block处理后的特征层
...
class DeepLab_moe(nn.Module):
    def __init__(self, num_classes, backbone="mobilenet", pretrained=True, downsample_factor=16, drop_path_rate=0.1, use_moe=True, expert_num=2, learnable_vec=0.5):
        super(DeepLab_moe, self).__init__()
        if backbone == "xception":
            ...
            self.backbone = Resnet(drop_path_rate)
            in_channels = 512
        ...
        self.cat_conv = nn.Sequential(
            SimplyDifferenceMoE(
                expert=SimplyExpert(in_channels=256+48, out_channels=256, drop_path_rate=drop_path_rate,
                                    use_moe=True),
                in_dim=304,
                expert_dim=256,
                ...
                proj_use_norm=True
            )if use_moe else SimplyExpert(in_channels=in_channels,out_channels=256, drop_path_rate=drop_path_rate, use_moe=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
    ...
    def forward(self, x):
        ...
        x = self.cls_conv(x)
        # 在上采样过程中将特征图调整为与输入图像尺寸大小一致，获得每个像素点的种类
...
if __name__ == "__main__":
    ...
    net = DeepLab_moe(num_classes=2, backbone='resnet', drop_path_rate=0.1, ).cuda()
    # net = DeepLab(num_classes=4, backbone="resnet", pretrained=False, downsample_factor=16).cuda()
    # net = swin_base_patch4_window12_384().cuda()
    import torchsummary

    # torchsummary.summary(net)
    print(net)
    image = torch.rand(2, 3,  800, 600).cuda()
