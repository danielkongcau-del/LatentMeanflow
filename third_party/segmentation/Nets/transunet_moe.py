...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.conv = SimplyDifferenceMoE(
            expert=SimplyExpert(in_channels=in_channels + skip_channels, out_channels=out_channels, drop_path_rate=drop_path_rate,
                                use_moe=True),
            in_dim=in_channels + skip_channels,
            expert_dim=out_channels,
            expert_num=expert_num,
            learnable_vec=learnable_vec,
            proj_use_norm=True
        ) if use_moe else SimplyExpert(in_channels=in_channels + skip_channels, out_channels=out_channels,
...
def config_transunet(img_size=512, n_classes=4,):
