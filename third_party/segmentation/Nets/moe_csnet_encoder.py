...
from Nets.moe_lab import MoE_FFN,FFNModule
...
class CSModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads: int = 32, drop_path_rate=0., moe_choice = "ab1" , choice='CNNSimpleAttention', patch_size = 16):
        super().__init__()
        if choice == 'CNNSimpleAttention':
            ...
            self.attention = SimpleAttention(out_channels, num_heads)
        self.ffn = MoE_FFN(out_channels, drop_path_rate, moe_choice=moe_choice)
        # Make skip path
...
if __name__ == '__main__':
    ...
    net = moe_CSNet(n_classes=2, num_heads=1, drop_path_rate=0.3, choice='CNNSimpleAttention', patch_size=16).cuda()
    print(net)
