...
class DWBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        skip_map = x =self.score(self.layernorm(self.DWconv(x).permute(0, 2, 3, 1)).permute(0, 3, 1, 2))+B2_shortcut
        x = self.DWreverce(self.gelu(self.DWextern(self.layernorm(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))))

        # x = self.DWreverce(self.gelu(self.DWextern(self.layernorm(self.DWconv(x).permute(0, 2, 3, 1) ).permute(0, 3, 1, 2) )))
        x = self.drop_path(x) + skip_map
        return x
...
if __name__ == '__main__':
    ...
    net = SegConvFormerV4(n_classes=4, num_heads=4, drop_path_rate=0.3).cuda()
