...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv = nn.Conv1d(in_channels, 3 * in_channels, 1)
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.shape
        x = x.reshape(b*(h//self.patch_size)*(w//self.patch_size), c, -1)
        qkv = self.qkv(self.norm(x.transpose(-1, -2)).transpose(-1, -2))
        attn = self.attention(qkv)
        attn = self.proj_out(attn)
        return (x + attn).reshape(b, c, h, w)
...
class SampleAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # Init layers
        self.norm = nn.LayerNorm(in_channels)  # 在DWconv后
        ...
        self.dwconv = nn.Conv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,padding='same',groups=in_channels,bias=True)
...
class FFNModule(nn.Module):
    def __init__(self,
        ...
        self.DWconv = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding='same', groups=in_channels)
        # self.DWextern = nn.Linear(out_channels, 4*out_channels)
        # self.DWreverce = nn.Linear(4*out_channels, out_channels)
        self.DWextern = nn.Conv2d(in_channels, 4 * in_channels, kernel_size=1, padding='same')
        self.DWreverce = nn.Conv2d(4 * in_channels, in_channels, kernel_size=1, padding='same')
        self.norm = nn.LayerNorm(in_channels)
    ...
    def forward(self, x):
        ...
        _B, _C, _H, _W = x.shape
        org_x = x
        x = self.DWreverce(self.gelu(self.DWextern(self.norm(self.DWconv(x).permute(0, 2, 3, 1)).permute(0, 3, 1, 2))))
        return self.drop_path(x)+org_x
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads: int = 32, drop_path_rate=0., choice='CNNSampleAttention', patch_size = 16):
        ...
        if choice == 'CNNSampleAttention':
            self.cnn = CNNModule(in_channels, out_channels)
            self.attention = SampleAttention(out_channels, num_heads)
        elif choice == 'CNNAttention':
            self.cnn = CNNModule(in_channels, out_channels)
            self.attention = Attention(out_channels, num_heads, patch_size=patch_size)
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_heads: int = 32, drop_path_rate=0., choice='CNNSampleAttention',patch_size = 16):
        ...
        self.attenBlock = Block(in_channels, out_channels, num_heads, drop_path_rate, choice, patch_size=patch_size)
...
class SampleOrMHSAUNet(nn.Module):
    def __init__(self, n_classes, num_heads: int = 32, drop_path_rate=0.1, choice='CNNSampleAttention', patch_size=16):
        super(SampleOrMHSAUNet, self).__init__()
        self.n_classes = n_classes

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, 4)]


        self.inc = Block(3, 64, num_heads, drop_path_rate,choice=choice,patch_size=patch_size)
        self.down1 = Down(64, 128, num_heads, dpr[0], choice=choice, patch_size=patch_size)  # in_channels, out_channels, num_heads: int = 32, drop_path_rate=0.
        self.down2 = Down(128, 256, num_heads, dpr[1], choice=choice, patch_size=patch_size)
        self.down3 = Down(256, 512, num_heads, dpr[2], choice=choice, patch_size=patch_size)
        self.down4 = Down(512, 512, num_heads, dpr[3], choice=choice, patch_size=patch_size)
        self.up1 = Up(1024, 256, dpr[0])  # in_channels, out_channels, num_heads: int = 32, drop_path_rate=0.
        self.up2 = Up(512, 128, dpr[1])
        self.up3 = Up(256, 64, dpr[2])
        self.up4 = Up(128, 64, dpr[3])
...
if __name__ == '__main__':
    ...
    net = SampleOrMHSAUNet(n_classes=2, num_heads=2, drop_path_rate=0.1, choice='Attention', patch_size=16).cuda()
    ...
    # x=torch.ones((1,32,56))
    # print(x.transpose(-1, -2).transpose(-1,-2).shape)
