...
from Nets.moe_lab import FFNModule
...
class DoubleConv_moe(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels, drop_path_rate=0.,choice="ab1"):
        super().__init__()
        self.covn1 = nn.Conv2d(in_channels,out_channels,kernel_size=3,padding=1)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.rule1 = nn.ReLU(inplace=True)
        self.ex1 = ExpertModule(out_channels,drop_path_rate)
        self.ex2 = ExpertModule(out_channels,drop_path_rate)
        self.ex3 = ExpertModule(out_channels, drop_path_rate)
        self.alpha = 0.5
        self.choice = choice
    def forward(self,x):
        x1 = self.covn1(x)
...
class UpConvModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, drop_path_rate=0.1, bilinear=True):
        ...
        self.conv = DoubleConv_moe(in_channels, out_channels, drop_path_rate)
...
class moe_CSNet_decoder(nn.Module):
    def __init__(self, n_classes, num_heads: int = 2, drop_path_rate=0.1, choice='CNNSimpleAttention', patch_size=16):
        super().__init__()
        self.n_classes = n_classes
...
if __name__ == '__main__':
    ...
    net = moe_CSNet_decoder(n_classes=2, num_heads=1, drop_path_rate=0.3, choice='CNNSimpleAttention', patch_size=16).cuda()
    print(net)
