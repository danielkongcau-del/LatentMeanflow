...
class SimpleAttention_weight_Nolayernorm(nn.Module):
    """[1,1,c]->[1,c]@[c,c]->[1,c]*[h*w,c]->[h,w,c]"""
    def __init__(self,
                 in_channels: int,
                 num_heads: int = 2
                 ):
        super(SimpleAttention_weight_Nolayernorm, self).__init__()
    ...
    def forward(self, x):
        ...
        x_temp = x_channel.mean((2, 3), keepdim=True).reshape(_B, _C, -1)
...
class SimpleAttention_weight_addlayernorDConv(nn.Module):
    """[1,1,c]->[1,c]@[c,c]->[1,c]*[h*w,c]->[h,w,c]"""
    def __init__(self,
                 in_channels: int,
                 num_heads: int = 2
                 ):
        super(SimpleAttention_weight_addlayernorDConv, self).__init__()
        self.in_channels: int = in_channels
        self.num_heads: int = num_heads
        self.scale: float = num_heads ** -0.5
        # Init layers
    ...
    def forward(self, x):
        ...
        # x[B,C,H,W]
        return x + skip
class SimpleAttention_weight_addlayernorlinear(nn.Module):
    """[1,1,c]->[1,c]@[c,c]->[1,c]*[h*w,c]->[h,w,c]"""
    def __init__(self,
                 in_channels: int,
                 num_heads: int = 2
                 ):
        super(SimpleAttention_weight_addlayernorlinear, self).__init__()
        self.in_channels: int = in_channels
        self.num_heads: int = num_heads
        self.scale: float = num_heads ** -0.5
        # Init layers
    ...
    def forward(self, x):
        ...
        # x_temp: [B,C,1]
        skip = x
        x_channel = self.norm(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
...
class SimpleAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # x[B,C,H,W]
        return x + skip
class SimpleAttention_noAV(nn.Module):
    """[1,1,c]->[1,c]@[c,c]->[1,c]*[h*w,c]->[h,w,c]"""
    def __init__(self,
                 in_channels: int,
                 num_heads: int = 2
                 ):
        super(SimpleAttention_noAV, self).__init__()
        self.in_channels: int = in_channels
        self.num_heads: int = num_heads
        self.scale: float = num_heads ** -0.5
        # Init layers
...
class SimpleAttention_noS(nn.Module):
    """[1,1,c]->[1,c]@[c,c]->[1,c]*[h*w,c]->[h,w,c]"""
    def __init__(self,
                 in_channels: int,
                 num_heads: int = 2
                 ):
        super(SimpleAttention_noS, self).__init__()
        self.in_channels: int = in_channels
        self.num_heads: int = num_heads
        self.scale: float = num_heads ** -0.5
        # Init layers
...
class CSModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads: int = 32, drop_path_rate=0., choice='ablation_weight_nolayernorm', patch_size = 16):
        ...
        # ------------------------------消融实验-weight-----------------------------------------------------------------------
        elif choice == 'ablation_weight_nolayernorm':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention_weight_Nolayernorm(out_channels, num_heads)
            self.ffn = FFNModule(out_channels, drop_path_rate)
        elif choice == 'ablation_weight_addlayernormlinear':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention_weight_addlayernorlinear(out_channels, num_heads)
            self.ffn = FFNModule(out_channels, drop_path_rate)
        elif choice == 'ablation_weight_addlayernormdconv':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention_weight_addlayernorDConv(out_channels, num_heads)
            self.ffn = FFNModule(out_channels, drop_path_rate)
        # ------------------------------消融实验-AVS-----------------------------------------------------------------------
        elif choice == 'ablation_module_noAVS':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = nn.Identity()
            self.ffn = FFNModule(out_channels, drop_path_rate)
        elif choice == 'ablation_module_AV':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention_weight_addlayernorDConv(out_channels, num_heads)
            self.ffn = FFNModule(out_channels, drop_path_rate)
        elif choice == 'ablation_module_S':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention_noAV(out_channels, num_heads)
            self.ffn = FFNModule(out_channels, drop_path_rate)
        #  ------------------------------消融实验-blockt-----------------------------------------------------------------------
        elif choice == 'ablation_block_cnnsiab':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = SimpleAttention(out_channels, num_heads)
            self.ffn = nn.Identity()
        elif choice == 'ablation_block_cnnseab':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = Attention(out_channels, num_heads)
            self.ffn = nn.Identity()
        elif choice == 'ablation_block_cnn':
            self.cnnblock = CNNBlock(in_channels, out_channels)
            self.attention = nn.Identity()
            self.ffn = nn.Identity()
...
if __name__ == '__main__':
    ...
    net = AblationCSNet(n_classes=4, num_heads=2, drop_path_rate=0.3, choice='CNNSimpleAttention', patch_size=16).cuda()
    print(net)
