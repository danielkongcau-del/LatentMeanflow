...
class GlobalBlock(nn.Module):
    ...
    一个很轻的全局通道增强：
    x: [B, C, H, W]
    全局池化 -> 小MLP -> Sigmoid -> 广播
    输出尺寸不变
    """
    def __init__(self, dim, pool=4, reduction=2):
        super().__init__()
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # 自适应池成小图，再平均，得到一个全局 token
        g = F.adaptive_avg_pool2d(x, (self.pool, self.pool))   # [B,C,p,p]
...
class Block(nn.Module):
    def __init__(self, dim=512, num_heads=16,  mlp_ratio=4, pool_ratio=16, drop=0., dilation=[3, 5, 7],
        ...
        self.attn = Mutilscal_MHSA(dim, num_heads=num_heads, atten_drop=drop, proj_drop=drop,
                                   dilation=dilation, pool_ratio=pool_ratio, fc_ratio=mlp_ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        mlp_hidden_dim = int(dim // mlp_ratio)
        self.mlp = E_FFN(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim,
...
class MAF(nn.Module):
    def __init__(self, dim, fc_ratio, dilation=[3, 5, 7], dropout=0., num_classes=6):
        ...
        self.head = nn.Sequential(
            SeparableConvBNReLU(dim, dim, kernel_size=3),
...
# =========================
# 这里是最终网络，加了 GlobalBlock
# =========================
class CMTFNetHA(nn.Module):
    def __init__(self,
                 encode_channels=[256, 512, 1024, 2048],
                 decode_channels=512,
                 dropout=0.1,
        ...
        # 新增的全局块，对应 res2, res3, res4
        # 你要原模原样地“加进去”，我就放这儿了
        self.g_res2 = GlobalBlock(dim=encode_channels[1], pool=4)
        self.g_res3 = GlobalBlock(dim=encode_channels[2], pool=4)
        self.g_res4 = GlobalBlock(dim=encode_channels[3], pool=2)
    ...
    def forward(self, x):
        ...
        # 这里插全局块（只处理高层三个）
        res2 = self.g_res2(res2)
        res3 = self.g_res3(res3)
        res4 = self.g_res4(res4)
...
if __name__ == '__main__':
    ...
    net = CMTFNet(num_classes=2).cuda()
    print(net)
