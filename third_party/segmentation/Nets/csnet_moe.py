...
class UpConvModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, drop_path_rate=0.1, bilinear=True, use_moe=True, expert_num=2, learnable_vec=0.5):
        super().__init__()
        ...
        self.conv = SimplyDifferenceMoE(
                expert=SimplyExpert(in_channels=in_channels,out_channels=out_channels, drop_path_rate=drop_path_rate, use_moe=True),
                in_dim=in_channels,
                expert_dim=out_channels, 
                expert_num=expert_num,
                learnable_vec=learnable_vec,
                proj_use_norm=True
        ) if use_moe else SimplyExpert(in_channels=in_channels,out_channels=out_channels, drop_path_rate=drop_path_rate, use_moe=False)
...
class CSNet_DMoE(nn.Module):
    def __init__(self, n_classes, num_heads: int = 2, drop_path_rate=0.1, choice='CNNSimpleAttention', patch_size=16, use_moe=True, expert_num=2, learnable_vec=0.5):
        super().__init__()
        ...
        self.up1 = UpConvModule(1024, 256, dpr[0], bilinear=True, use_moe=use_moe, expert_num=expert_num, learnable_vec=learnable_vec)
        self.up2 = UpConvModule(512, 128, dpr[1], bilinear=True,use_moe=use_moe, expert_num=expert_num, learnable_vec=learnable_vec)
        self.up3 = UpConvModule(256, 64, dpr[2], bilinear=True,use_moe=use_moe, expert_num=expert_num, learnable_vec=learnable_vec)
        self.up4 = UpConvModule(128, 64, dpr[3], bilinear=True,use_moe=use_moe, expert_num=expert_num, learnable_vec=learnable_vec)
...
if __name__ == '__main__':
    ...
    net = CSNet_DMoE(n_classes=4, num_heads=2, drop_path_rate=0.3, choice='CNNSimpleAttention', patch_size=16).cuda()
    print(net)
