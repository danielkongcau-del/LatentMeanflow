...
def train(args, logger):
    ...
    model = choose_net(args.net_name, args.out_channels, img_size=img_size)
...
def do_train(args):
    ...
    if world_size <= 0 or rank < 0:
        raise RuntimeError("Please launch with torchrun, e.g. torchrun --nproc_per_node=1 train_linux.py ...")
