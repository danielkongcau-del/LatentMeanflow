...
def train(args):
    ...
    # Prepare save dir
    save_dir = './result_labeldit_D-D/pretrained/' + args.save_suffix + '-' + args.net_name + '-h' + str(train_set[0][0].shape[1]) + 'w' \
    ...
    if args.pt_dir is not None and args.pt_dir != '':
        msg = net.load_state_dict(torch.load(args.pt_dir, map_location='cpu'), strict=False)
        print(msg)
        print('Loaded model pretrained weights:', args.pt_dir)
...
def search_train(args):
    ...
    args.batch_size = 4
    args.gpu = 0
    args.pt_dir = ''   # model pretrained weights
    # train_net_names only support ['segformer', 'unet', 'deeplabv3-resnet', 'csnet', 'munet', 'transunet']
    train_net_names = ['unet_Dmoe']
    save_suffix = 'aug'
    task = 0
