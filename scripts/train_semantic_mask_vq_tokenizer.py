...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train the project-layer mask-only discrete tokenizer. "
            "This route reconstructs semantic_mask only and is the tokenizer half of the planned "
            "mask-only discrete tokenizer -> token prior mainline."
        )
    )
    ...
    parser.add_argument(
        "--run-test",
        action="store_true",
        help="Opt in to Trainer.test() after fit. Disabled by default for this tokenizer route.",
    )
    parser.add_argument(
...
def resolve_run_name(config_path):
    ...
    return stem if stem else "semantic_mask_vq_tokenizer"
...
def print_learning_rate_summary(*, config, config_path, args):
    if config is None or config_path is None:
        print(
            f"[train_semantic_mask_vq_tokenizer] scale_lr={bool(args.scale_lr)}. "
    ...
    print(
        f"[train_semantic_mask_vq_tokenizer] base_lr={base_lr:.2e}, scale_lr={bool(args.scale_lr)}, "
