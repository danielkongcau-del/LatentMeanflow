...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "maskgit_palette_vq_tokenizer_imagenet_f16_256.yaml"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Fine-tune the pretrained MaskGIT image VQ tokenizer on palette-rendered semantic_mask data. "
            "This is a diagnostic stage-1 route for testing whether a shorter image-token sequence is more "
            "prior-friendly than the project-layer semantic VQ tokenizer."
        )
    )
    ...
    parser.add_argument(
        ...
        const=True,
        default=True,
        help="true reproduces the vendored effective-lr scaling by accumulate_grad_batches * ngpu * batch_size.",
    )
    parser.add_argument("--run-test", action="store_true")
    parser.add_argument("--allow-config-override", action="store_true")
    parser.add_argument("--allow-dotlist-override", action="store_true")
...
def resolve_config(args):
    return args.config if args.config is not None else DEFAULT_CONFIG
...
def resolve_run_name(config_path):
    ...
    return stem if stem else "maskgit_palette_vq_tokenizer"
...
def validate_resume_request(args, config_path=None, resume_logdir=None):
    ...
    if has_any_dotlist_style_override(args) and not args.allow_dotlist_override:
        raise ValueError(
            "Resume with dotlist-style overrides is not allowed by default. Safe resume should pass only --resume. "
            "If you truly want to inject dotlist overrides into a resumed run, add --allow-dotlist-override explicitly."
        )
    if args.config is not None and not args.allow_config_override:
        ...
        if not resume_run_matches_config(resume_logdir, config_path):
            raise ValueError(
                ...
                f"'{Path(config_path).name}'. Safe resume should pass only --resume."
            )
...
def print_learning_rate_summary(*, config, config_path, args):
    if config is None or config_path is None:
        print(
            f"[train_maskgit_palette_vq_tokenizer] scale_lr={bool(args.scale_lr)}. "
    ...
    print(
        f"[train_maskgit_palette_vq_tokenizer] base_lr={base_lr:.2e}, scale_lr={bool(args.scale_lr)}, "
