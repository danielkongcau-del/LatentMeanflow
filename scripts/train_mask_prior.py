...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "latent_alphaflow_mask_prior_unet.yaml"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train the project-layer unconditional semantic-mask route p(semantic_mask). "
            "This baseline stays in direct mask space and does not use any tokenizer."
        )
    )
...
def resolve_run_name(config_path):
    ...
    return stem if stem else "latent_alphaflow_mask_prior_unet"
...
def print_learning_rate_summary(*, config, config_path, args):
    if config is None or config_path is None:
        print(
            f"[train_mask_prior] scale_lr={bool(args.scale_lr)}. "
    ...
    print(
        f"[train_mask_prior] base_lr={base_lr:.2e}, scale_lr={bool(args.scale_lr)}, "
