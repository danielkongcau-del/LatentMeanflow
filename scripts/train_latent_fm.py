...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the latent flow matching baseline on semantic tokenizer latents.")
...
def build_command(args, tokenizer_ckpt):
    cmd = [
        sys.executable,
        str(LDM_LAUNCHER),
        "-t",
...
def main():
    ...
    cmd = build_command(args, tokenizer_ckpt)
