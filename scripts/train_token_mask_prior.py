...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train the project-layer unconditional token-code mask generator on top of the frozen balanced "
            "discrete VQ semantic-mask tokenizer."
        )
    )
...
def resolve_run_name(config_path):
    ...
    return stem if stem else "token_mask_prior_vq_sit"
...
def print_learning_rate_summary(*, config, config_path, args):
    if config is None or config_path is None:
        print(
            f"[train_token_mask_prior] scale_lr={bool(args.scale_lr)}. "
    ...
    print(
        f"[train_token_mask_prior] base_lr={base_lr:.2e}, scale_lr={bool(args.scale_lr)}, "
