...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Sample unconditional semantic masks from the project-layer p(semantic_mask) baseline. "
...
def _resolve_nfe_dirs(root, requested_nfe_values):
    resolved = []
    for nfe in requested_nfe_values:
        path = root / f"nfe{int(nfe)}"
        if path.exists():
            resolved.append((int(nfe), path))
    if resolved:
        return resolved
    auto_dirs = []
    for path in sorted(root.glob("nfe*")):
        try:
