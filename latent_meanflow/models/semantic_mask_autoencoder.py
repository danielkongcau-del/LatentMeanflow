...
class SemanticMaskLoss(nn.Module):
    ...
    def forward(self, mask_index, mask_logits, posterior):
...
class SemanticMaskAutoencoder(pl.LightningModule):
    def __init__(
        self,
        ddconfig,
        lossconfig,
        ...
        if getattr(self.decoder, "give_pre_end", False):
            # The project-layer mask head consumes decoder pre-end features directly.
    ...
    def _infer_mask_index_from_onehot(self, mask_onehot):
        ...
        if self.loss.ignore_index is None:
            if not torch.all(has_signal):
                raise ValueError(
                    "mask_onehot contains zero-sum positions, but ignore_index is not configured."
                )
            return mask_index
    ...
    def get_input(self, batch):
        ...
        if not has_mask_index and not has_mask_onehot:
            raise KeyError(
                f"SemanticMaskAutoencoder requires '{self.mask_index_key}' and/or '{self.mask_onehot_key}' in batch."
            )
    ...
    def forward(self, batch, sample_posterior=None):
        ...
        posterior = self.encode(mask_onehot)
        latent_mode = posterior.mode()
        if sample_posterior is None:
            sample_posterior = self.sample_posterior
        z = posterior.sample() if sample_posterior else latent_mode
        mask_logits = self.decode(z)
        recon_mask_index = torch.argmax(mask_logits, dim=1)
        recon_mask_onehot = F.one_hot(recon_mask_index, num_classes=self.num_classes).permute(0, 3, 1, 2).float()
        mask_probs = torch.softmax(mask_logits, dim=1)
        total_loss, loss_dict = self.loss(mask_index, mask_logits, posterior)
        return {
            "z": z,
